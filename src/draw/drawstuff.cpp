...
static bool suspend_shadows = false;  // true if drawing shadows must be suspended
...
static void drawGround(float pos_offset[3])
{
    float pos_offset_x = round(pos_offset[0]);
    float pos_offset_y = round(pos_offset[1]);
...
void dsDrawFrame (int width, int height, dsFunctions *fn, int pause, int singlestep)
{
  ...
  if(*(fn->drawScene))
  {
	  ...
	  // clear the window
	  glClearColor (GROUND_R, GROUND_G, GROUND_B, 0);
  ...
  if (fn->step) fn->step (pause,singlestep);
}
